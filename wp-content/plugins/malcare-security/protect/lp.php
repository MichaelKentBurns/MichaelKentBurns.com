...
class MCProtectLP {
	...
	private $mode              = MCProtectLP::MODE_DISABLED;
	...
	private $category          = MCProtectLP::CATEGORY_ALLOWED;
	private $username          = '';
	private $message           = '';
	...
	private function __construct($request, $config, $brand_name) {
		...
		$this->ipstore = new MCProtectIpstore();
		$this->logger = new MCProtectLogger(MCProtectLP::TABLE_NAME);
	...
	public static function uninstall() {
		MCProtect::$db->dropBVTable(MCProtectLP::TABLE_NAME);
	}
	...
	private function getCaptchaLink() {
		$account = MCAccount::apiPublicAccount(MCProtect::$settings);
	...
	private function getAllowLoginsTransient() {
		return MCProtect::$settings->getTransient('bvlp_allow_logins');
	}

	private function getBlockLoginsTransient() {
		return MCProtect::$settings->getTransient('bvlp_block_logins');
	}
	...
	private function isProtecting() {
		return $this->mode === MCProtectLP::MODE_PROTECT;
	}

	private function isActive() {
		return $this->mode !== MCProtectLP::MODE_DISABLED;
	}
	...
	private function isUnBlockedIP() {
		$transient_name = MCProtectLP::UNBLOCK_IP_TRANSIENT_PREFIX . $this->ip;
		$attempts = MCProtect::$settings->getTransient($transient_name);

		if ($attempts && $attempts > 0) {
			MCProtect::$settings->setTransient($transient_name, $attempts - 1, 600 * $attempts);
			return true;
		}
	...
	private function isLoginBlocked() {
		if ($this->getAllowLoginsTransient() ||
				($this->getLoginCount(MCProtectLP::LOGIN_STATUS_FAILURE, null, $this->all_blocked_gap) < $this->block_all_limit)) {
			return false;
		}
	...
	private function terminateLogin() {
		...
		$this->log(MCProtectLP::LOGIN_STATUS_BLOCKED);
	...
	public function loginInit($user, $username = '', $password = '') {
		if ($this->isUnBlockedIP()) {
			$this->category = MCProtectLP::CATEGORY_UNBLOCKED;
		} else {
			$failed_attempts = $this->getLoginCount(MCProtectLP::LOGIN_STATUS_FAILURE,
																							$this->ip, $this->failed_login_gap);

			if ($this->isWhitelistedIP()) {
				$this->category = MCProtectLP::CATEGORY_BYPASSED;
			} elseif (MCProtectUtils::isPrivateIP($this->ip)) {
				$this->category = MCProtectLP::CATEGORY_PRIVATEIP;
			} elseif ($this->isBlacklistedIP()) {
				$this->category = MCProtectLP::CATEGORY_BLACKLISTED;
				$this->terminateLogin();
			} elseif ($this->isKnownLogin()) {
				$this->category = MCProtectLP::CATEGORY_BYPASSED;
			} elseif ($this->isLoginBlocked()) {
				$this->category = MCProtectLP::CATEGORY_ALL_BLOCKED;
				$this->terminateLogin();
			} elseif ($failed_attempts >= $this->temp_block_limit) {
				$this->category = MCProtectLP::CATEGORY_TEMP_BLOCK;
				$this->terminateLogin();
			} elseif ($failed_attempts >= $this->captcha_limit) {
				$this->category = MCProtectLP::CATEGORY_CAPTCHA_BLOCK;
				$this->terminateLogin();
			}
		}
	...
	public function loginFailed($username) {
		...
		$this->log(MCProtectLP::LOGIN_STATUS_FAILURE);
	}

	public function loginSuccess($username) {
		$this->username = $username;
		$this->message = 'Login Success';
		$this->log(MCProtectLP::LOGIN_STATUS_SUCCESS);
	}

	private function isKnownLogin() {
		return $this->getLoginCount(MCProtectLP::LOGIN_STATUS_SUCCESS,
																$this->ip, $this->success_login_gap) > 0;
	}

	private function getLoginCount($status, $ip, $gap) {
		$table = MCProtect::$db->getBVTable(MCProtectLP::TABLE_NAME);
		$query_str = "SELECT COUNT(*) as count from `$table` WHERE status=%d && time > %d";
		$query_args = array($status, ($this->time - $gap));

		$query = MCProtect::$db->prepare($query_str, $query_args);
		if ($ip) {
			$query .= MCProtect::$db->prepare(" && ip=%s", $ip);
		}

		$rows = MCProtect::$db->getResult($query);
		if (!$rows) {
			return 0;
		}
