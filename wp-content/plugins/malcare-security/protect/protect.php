...
if (!class_exists('MCProtect_V545')) :
...
class MCProtect_V545 {
	...
	public static function init($mode) {
		if ($mode == MCProtect_V545::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = MCProtectUtils_V545::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(MCProtect_V545::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new MCProtectRequest_V545($request_ip_header);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			MCProtectFW_V545::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			MCProtectFW_V545::getInstance($mode, $request, $fw_config, $brand_name)->init();
			MCProtectLP_V545::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		MCProtectIpstore_V545::uninstall();
		MCProtectFW_V545::uninstall();
		MCProtectLP_V545::uninstall();

		MCProtect_V545::removeWPPrepend();
		MCProtect_V545::removePHPPrepend();
		MCProtect_V545::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		MCProtectUtils_V545::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		MCProtect_V545::removeHtaccessPrepend();
		MCProtect_V545::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		MCProtectUtils_V545::fileRemovePattern(dirname(ABSPATH) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		MCProtectUtils_V545::fileRemovePattern(dirname(ABSPATH) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		MCProtectUtils_V545::rrmdir($mc_data_dir);
	}
}
endif;