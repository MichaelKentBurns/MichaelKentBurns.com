...
if (!class_exists('CWMGRRespStream')) :

	class CWMGRStream extends CWMGRCallbackBase {
		public $bvb64stream;
		public $bvb64cksize;
		public $checksum;
		...
		public static function startStream($account, $request) {
			...
			$stream = new CWMGRRespStream($request);
			if ($request->isAPICall()) {
				$stream = new CWMGRHttpStream($request);
				if (!$stream->connect()) {
					$apicallstatus = array(
...
class CWMGRRespStream extends CWMGRStream {
	public $bvboundry;
...
class CWMGRHttpStream extends CWMGRStream {
	var $user_agent = 'CWMGRHttpStream';
	var $host;
	var $port;
	var $timeout = 20;
	var $conn;
	...
	public function getResponse() {
		...
		while (!feof($this->conn)) {
			...
			if (1 == $state) {
				if (!CWMGRHelper::safePregMatch('/HTTP\/(\\d\\.\\d)\\s*(\\d+)\\s*(.*)/', $line, $m)) {
				...
				if (!CWMGRHelper::safePregMatch('/([^:]+):\\s*(.*)/', $line, $m)) {
