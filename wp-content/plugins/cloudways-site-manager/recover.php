...
if (!class_exists('CWMGRRecover')) :
	class CWMGRRecover {
		...
		public static function defaultSecret($settings) {
			...
			if (empty($secret)) {
				$secret = CWMGRRecover::refreshDefaultSecret($settings);
			}
			return $secret;
		}
		...
		public static function refreshDefaultSecret($settings) {
			...
			$key_details = array();
			$key_details["key"] = CWMGRAccount::randString(32);
			...
			if (empty(self::configSalt())) {
				$key_details["salt"] = CWMGRAccount::randString(self::SALT_LENGTH);
			}
		...
		public static function find($settings, $pubkey, $tag = null) {
			...
			$account = new CWMGRAccount($settings, $pubkey, $secret);
			return $account;
		}
	}
endif;
