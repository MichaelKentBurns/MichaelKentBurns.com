...
if (!class_exists('CWMGRHelper')) :
	class CWMGRHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!CWMGRWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = CWMGRWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					CWMGRWPFileSystem::getInstance()->putContents($fname, $modified_content,
							CWMGRWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
