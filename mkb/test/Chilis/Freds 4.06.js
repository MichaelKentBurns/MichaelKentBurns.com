...
**      but allow lines to remain highlighted, perhaps
**      with different colors.
**    * Add an action to decide who's turn it is to pay
**      after selecting those Freds present by clicking
**      on their column headings.
**    * Provide a way to add a new entry to the table,
**      selecting those Freds present and indicating
**      who paid for themselves, and who paid for the
**      remaining present.
**    * How do we take a new entry and get it added to
**      the fredYYYY.js file we're currently using?
**
** Versions:
**    4.01 - Initial coding 
...
**    4.05 - Add Weekday column
**    4.06 - Make reverse table order the default
*/
Version            = "4.06"
//owMenuHtml       = 1
//owGameHtml       = 1
...
puzzleType         = ""
menuFunction       = "menuItemFredsLunch"
playOffset         = 100
gameWinHigh        = 900
gameWinWide        = 1400
boardOrder         = 0
...
gameHtml = new Array (
   ...
   '<table border="1" cellpadding="2" id="BOARD">',
   '<tr>',
   '   <th align="center" width="50">Date</th>',
   '   <th align="center" width="50">&nbsp;DateText&nbsp;</th>',
   '   <th align="center" width="50">&nbsp;Weekday&nbsp;</th>',
   '   <th align="center" width="10">&nbsp;</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(1)">Fred</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(2)">Fred</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(3)">Fred</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(4)">Fred</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(5)">Fred</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(6)">Fred</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(7)">Fred</th>',
   '   <th align="center" width="20" name="FRED" onclick="clickColumn(8)">Fred</th>',
   '   <th align="center" width="10">&nbsp;</th>',
   ...
   '   <td align="center"><font size="4" id="optionXtraRowB">&nbsp;</font></td>',
   '   <td>Row balance top and bottom</td></tr>',
   '<tr><td valign="bottom" colspan="3">&nbsp;<br><b>Actions</b></td></tr>',
   '<tr><td>&nbsp;&nbsp;</td><td colspan="2" onclick="resetBoard()"><button>Reset the board</button></td></tr>',
   '<tr><td>&nbsp;&nbsp;</td><td colspan="2" onclick="reverseBoard()"><button>Reverse the table order</button></td></tr>',
...
function populateBoard()
{
   ...
   {
      fredCELL[i].textContent = nbsp + nbsp + freds[i] + nbsp + nbsp
   }
   ...
   list[1] = "Jan 1"
   list[2] = getWeekday(showYear.substr(-2),"01","01")
   list[3] = nbsp
   n =  3
   ...
   for( i=0; i< (lunches.length/(freds.length+2)); i++ )
   {
      ...
      list[2] = getWeekday(showYear.substr(-2),mm,dd)
      list[3] = nbsp
      n =  3
      sum = 0
      for( x=0; x<freds.length; x++ )
   ...
   list[1] = "Dec 31"
   list[2] = getWeekday(showYear.substr(-2),"12","31")
   list[3] = nbsp
   n =  3
   for( x=0; x<freds.length; x++ )
   {
   ...
   /* Make the reverse table order the new default */
   reverseBoard()
...
function getWeekday(yy,mm,dd)
{
   var  dn
   var  day

   debug("Entering getWeekday("+yy+","+mm+","+dd+")")
   ...
   debug("Exiting getWeekday() with '"+day+"'")
   return day
}
...
function clickColumn(n)
{
   ...
   debug("Entering clickColumn("+n+")")

   /* Clear the messages */
   msg1text.data = nbsp
   msg2text.data = nbsp

   /* Ctrl-Alt means jump into debug */
   ...
   {
      msg1text.data = "NOTE:"
      msg2text.data = "Column highlighting not yet implemented."
   }


   debug("Exiting clickColumn()")
}
