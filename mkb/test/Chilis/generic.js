...
/*
** Bugs:
**   * genericFlash# functions not working with only
**     puzzle window showing
**   * ?
**
** To Do:
...
**    4.02   About title
**    4.03   Accommodate Ctrl-Alt-# answer flashing
**    4.04   Look for Ctrl and/or Alt in *Border()
**    4.05   Complete document title, source, and name
**    4.06   Debug menu toggles
**    4.07   Correct weekday and add extra letters after menu item
**    4.08   Header and footer message tables and table ID tool tips
*/
...
char2612       = String.fromCharCode(9746) // &#9746; == &#x2612; == X in square
checkDate      = new Date
...
/***
     ...
     { noMenuCols = 5 }
   else if (menunum <= 270)   //  38 - 45 rows
     { noMenuCols = 6 }
   else if (menunum <= 315)   //  39 - 45 rows
     { noMenuCols = 7 }
   else if (menunum <= 360)   //  40 - 45 rows
     { noMenuCols = 8 }
   else // (menunum <= 405)   //  41 - 45 rows
     { noMenuCols = 9 }
***/
   else                       //  Maximum of 43 rows
     {
       noMenuCols = Math.ceil(menunum/43)
     }
/**/
// alert("menunum is "+menunum+" and noMenuCols is "+noMenuCols)
   ...
   */
   for( i=0; i<menunum; i++ )
     {
      gamelink[i] = menuItem(menulist[i])
...
function menuItem(fileName)
{
   ...
   if ( (fileName.substring(2,3) == '0') ||
        (fileName.substring(2,3) == '1') ||
        (fileName.substring(2,3) == '2') ||
        (fileName.substring(2,3) == '3') ||
        (fileName.substring(2,3) == '4') ||
        (fileName.substring(2,3) == '5') )
   {
      yy = fileName.substring(2,4)
      mm = fileName.substring(4,6)
      dd = fileName.substring(6,8)
      ...
      day = dNam[dn]
      mon = mNam[mm-1]
      retItem = mon + nbsp + dd + "," + nbsp + "20" + yy
                    + nbsp +      "(" + day  + ")"
...
//    alert(oldDstr+" / "+newDstr)
      ...
      /* If there are characters after the date, include them */
      if (fileName.length > 8)
      {
         r = 8
         p = 1
         while(fileName.length > r)
         {
            letter = fileName.substr(r,1)
            if (1 == p)
               retItem += ' [' + letter
            else
               retItem += ' '  + letter
   ...
   else if (menuFunction)
   {
      eval("retItem = "+menuFunction+"(fileName)")
   }
   else
   {
      retItem = fileName + puzzleType
   }
...
// alert("Entering debugDOM("+v+")")
   ...
   {
      debugWindow.document.write('<p>Document Head.<br>')
      dumpTree(document.head)
      debugWindow.document.write('<br>End of Document Head.')
   }
   else if (-4 == v)
   {
      debugWindow.document.write('<p>Document HTML Element.<br>')
      dumpTree(document.documentElement)
      debugWindow.document.write('<br>End of Document HTML Element.')
   }
   else if (0 < v)
   {
...
function genericKeyDn(evt)
{
   ...
   /* Puzzle specific action */
   if (flashKeyDn)
     {
        eval(flashKeyDn+"(evt)")
     }

   /* Handle CTRL-ALT-# Flashing */
   if ( (keyFlags & KEYFLAG_CTRL) &&
        (keyFlags & KEYFLAG_ALT) )
     {
      ...
      /* Different ways to find the digit 1 on the keyboard */
      if (flashFunction1)
        {
         if (theKey == 35 || theKey == 49 || theKey ==  97)
           {
            eval(flashFunction1+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 2 on the keyboard */
      if (flashFunction2)
        {
         if (theKey == 40 || theKey == 50 || theKey ==  98)
           {
            eval(flashFunction2+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 3 on the keyboard */
      if (flashFunction3)
        {
         if (theKey == 34 || theKey == 51 || theKey ==  99)
           {
            eval(flashFunction3+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 4 on the keyboard */
      if (flashFunction4)
        {
         if (theKey == 37 || theKey == 52 || theKey == 100)
           {
            eval(flashFunction4+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 5 on the keyboard */
      if (flashFunction5)
        {
         if (theKey == 12 || theKey == 53 || theKey == 101)
           {
            eval(flashFunction5+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 6 on the keyboard */
      if (flashFunction6)
        {
         if (theKey == 39 || theKey == 54 || theKey == 102)
           {
            eval(flashFunction6+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 7 on the keyboard */
      if (flashFunction7)
        {
         if (theKey == 36 || theKey == 55 || theKey == 103)
           {
            eval(flashFunction7+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 8 on the keyboard */
      if (flashFunction8)
        {
         if (theKey == 38 || theKey == 56 || theKey == 104)
           {
            eval(flashFunction8+"()")
            flashAgain = 1
           }
      ...
      /* Different ways to find the digit 9 on the keyboard */
      if (flashFunction9)
        {
         if (theKey == 33 || theKey == 57 || theKey == 105)
           {
            eval(flashFunction9+"()")
            flashAgain = 1
           }
...
function genericTableDesc()
{
   ...
   tableID.push("PAGE");            tableDesc.push("Enclosing"+nbsp+"table"+nbsp+"for"+nbsp+"the"+nbsp+"web"+nbsp+"page")
   tableID.push("BOARD");           tableDesc.push(puzzleName + lcType + " board")
   tableID.push("BOARDMSG1");       tableDesc.push("Puzzle header messages table")
   tableID.push("BOARDMSG2");       tableDesc.push("Puzzle footer messages table")
   tableID.push("ENCLOSEBOARD");    tableDesc.push("Enclosing"+nbsp+"table"+nbsp+"for"+nbsp+"the"+nbsp+"puzzle"+nbsp+"pieces")
   tableID.push("ENCLOSESETTINGS"); tableDesc.push("Enclosing table for Settings")
   tableID.push("SETTINGS");        tableDesc.push("Settings table")
   tableID.push("SETTINGS1");       tableDesc.push("Settings table left sleeve")
   tableID.push("SETTINGS2");       tableDesc.push("Settings table right sleeve")
   tableID.push("COLOR");           tableDesc.push("Color selector table")
   tableID.push("COLOR1");          tableDesc.push("Color selector table")
   tableID.push("COLOR2");          tableDesc.push("Color assignment table")
   tableID.push("GRIDCOLOR");       tableDesc.push("Color scheme selector table")
   tableID.push("TEXTCOLOR");       tableDesc.push("Text color selector table")
   tableID.push("MESSAGES");        tableDesc.push("Messages table")
   tables = tableID.length
...
function genericScan(fileName)
{
   ...
   if ( (fileName.substring(2,3) == '0') ||
        ...
        (fileName.substring(2,3) == '5') )
   {
      fn = fileName.substr(1,1)
      yy = fileName.substr(2,2)
      mm = fileName.substr(4,2)
      dd = fileName.substr(6,2)
      if        (fn == 'a')
         gameBanner  = "Austin American-Statesman"
      else   if (fn == 'd')
         gameBanner  = "Dallas Morning News"
      else   if (fn == 'u')
         gameBanner  = "USA Today"
      else
         gameBanner  = "Mystery source '"  +  fn   + "'"
      gameBanner    += " " + mm + "/" + dd + "/20" + yy
      document.title =       mm + "/" + dd + "/20" + yy
   }
   else
   {
      gameBanner     = fileName
   ...
   document.title += " " + puzzleTitle + puzzleType
   puzzleBanner    =       puzzleTitle + puzzleType
   settingsTitle   = "JavaScript " + puzzleName    + " Version " + Version
   aboutTitle      = settingsTitle + "\n"
   ...
   /* Fill in the values for those pieces */
   bannerH1.textContent = puzzleBanner
   bannerH3.textContent = gameBanner
   menuTitle.textContent = settingsTitle
   ...
   /* Find and initialize the debug menu */
   charTracePlay = document.getElementById("tracePlay")
   if (charTracePlay)
   {
      if (debugTracePlay)
           charTracePlay.textContent = char2611
      else
           charTracePlay.textContent = char2610
   }
   charTraceText = document.getElementById("traceText")
   if (charTraceText)
   {
      if (debugTraceText)
           charTraceText.textContent = char2611
      else
           charTraceText.textContent = char2610
   }
   charTraceFind = document.getElementById("traceFind")
   if (charTraceFind)
   {
      if (debugTraceFind)
           charTraceFind.textContent = char2611
      else
           charTraceFind.textContent = char2610
   }
   charTraceMenu = document.getElementById("traceMenu")
   if (charTraceMenu)
   {
      if (debugTraceMenu)
           charTraceMenu.textContent = char2611
      else
           charTraceMenu.textContent = char2610
   }
   charDebugWndw = document.getElementById("debugWndw")
   if (charDebugWndw)
   {
      if (debugWindow)
           charDebugWndw.textContent = char2611
      else
           charDebugWndw.textContent = char2610
   }
...
function populateBorderToggles()
{
   ...
   for( n=0; n<tlist.length; n++ )
   {
      ...
      document.getElementById("border"+x+"title").setAttribute('title','Table '+x+' Id="'+tabid+'"')
      if ( ('PAGE'            == tabid) ||
           ('ENCLOSESETTINGS' == tabid) ||
           ('SETTINGS'        == tabid) ||
           ('SETTINGS2'       == tabid) )
         borderNhide[n] = 1
      else
      ...
      for( i=0; i<tableID.length; i++ )
      {
         if ( tableID[i] == tabid )
         {
            borderTitle[n].data = x + ":" + nbsp + tableDesc[i]
         }
      }
   }
...
function resetBorders()
{
   ...
   {
      /* Determine what new state */
      if (keyFlags & KEYFLAG_CTRL)
         borderValue[n] = 1
      else if (keyFlags & KEYFLAG_ALT)
         borderValue[n] = 0
      else
         borderValue[n] = borderDefault[n]
      ...
      if (borderValue[n] & 2)
      {
         borderCheck[n].data = char2612
         borderTable[n].setAttribute('hidden',1)
      }
      else
      {
         borderTable[n].removeAttribute('hidden')
      }
   }
...
function toggleTracePlay(n)
{
   debug("Entering toggleTracePlay("+n+")")
   ...
   debug("Exiting toggleTracePlay()")
}

function toggleTraceText(n)
{
   debug("Entering toggleTraceText("+n+")")
   ...
   debug("Exiting toggleTraceText()")
}

function toggleTraceFind(n)
{
   debug("Entering toggleTraceFind("+n+")")
   ...
   debug("Exiting toggleTraceFind()")
}

function toggleTraceMenu(n)
{
   debug("Entering toggleTraceMenu("+n+")")
   ...
   debug("Exiting toggleTraceMenu()")
}

function toggleDebugWndw(n)
{
   debug("Entering toggleDebugWndw("+n+")")
   ...
   else
   {
      ...
      debugWindow = window.open("","Debug",
         ...
         "scrollbars=yes,"+
         "resizable=yes")
      debugWindow.document.write("<h1>Debug Window</h1>");
   }

   debug("Exiting toggleDebugWndw()")
}
...
function scanTable(fileName)
{
   ...
   /* Generic specific key management */
   flashFunction0 = "genericFlash0"
   flashFunction1 = "genericFlash1"
   flashFunction2 = "genericFlash2"
   flashFunction3 = "genericFlash3"
   flashFunction4 = "genericFlash4"
   flashFunction5 = "genericFlash5"
   flashFunction6 = "genericFlash6"
   flashFunction7 = "genericFlash7"
   flashFunction8 = "genericFlash8"
   flashFunction9 = "genericFlash9"
...
function genericFlash0()
{
   alert("You have pressed Ctrl-Alt-0")
}

function genericFlash1()
{
   alert("You have pressed Ctrl-Alt-1")
}

function genericFlash2()
{
   alert("You have pressed Ctrl-Alt-2")
}

function genericFlash3()
{
   alert("You have pressed Ctrl-Alt-3")
}

function genericFlash4()
{
   alert("You have pressed Ctrl-Alt-4")
}

function genericFlash5()
{
   alert("You have pressed Ctrl-Alt-5")
}

function genericFlash6()
{
   alert("You have pressed Ctrl-Alt-6")
}

function genericFlash7()
{
   alert("You have pressed Ctrl-Alt-7")
}

function genericFlash8()
{
   alert("You have pressed Ctrl-Alt-8")
}

function genericFlash9()
{
   alert("You have pressed Ctrl-Alt-9")
}
